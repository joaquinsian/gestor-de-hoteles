...
function crearReservaciones(req, res){
    ...
    if(params.nombre && params.apellido){
        ...
        reservacion.fecha_inicio_de_reservacion = params.fecha_inicio_de_reservacion;
        ...
        Reservacion.find({_id: reservacion._id}).exec((err, resencontrada)=>{
            if(err) return res.status(500).send({ mensaje: ' Error al realizar la reservacion'});

            if(resencontrada.lenght > 0){
                return res.status(500).send({ mensaje : 'habitacion reservada actualmente'})
            }else{
                reservacion.save((err, reservacionSave)=>{
                    if(err){ res.status(500).send({mensaje: 'la habitacion se encuentra reservada actualmente'});
